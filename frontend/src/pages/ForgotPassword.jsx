...
import AuthService from '../services/authServices'; // Adjust the import path as needed
...
const ForgotPassword = () => {
    ...
    const handleSendOtp = async (e) => {
        ...
        try {
            const result = await AuthService.forgotPasswordSendOTP(formData.email.toLowerCase());
            ...
            if (result.success) {
                ...
                if (result.status === 404 || result.message?.includes('No account found')) {
    ...
    const handleVerifyOtp = async (e) => {
        ...
        try {
            const result = await AuthService.forgotPasswordVerifyOTP(formData.email.toLowerCase(), formData.otp);
            ...
            if (result.success) {
                ...
                setErrors({ general: result.message || 'Invalid or expired OTP. Please try again.' });
            }
        } catch (error) {
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 flex items-center justify-center px-4 py-6 sm:px-6 lg:px-8 relative overflow-hidden">
          ...
          transition-all duration-700 hover:scale-[1.02] group">
                            ...
                            <div className="flex items-center space-x-4">
                                {[1, 2, 3].map((step) => (
                                    <div key={step} className="flex items-center">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 ${currentStep >= step
                                        ...
                                        {step < 3 && (
                                            <div className={`w-8 h-0.5 mx-2 transition-all duration-300 ${currentStep > step ? 'bg-sky-400' : 'bg-slate-600'
                                                }`}></div>
                                        )}
                                    </div>
                                ))}
                    ...
                    disabled:shadow-[0_0_15px_rgba(14,165,233,0.15),0_4px_8px_rgba(0,0,0,0.2)]"
                        ...
                        shadow-[inset_0_3px_6px_rgba(0,0,0,0.4),0_2px_4px_rgba(14,165,233,0.1),0_0_10px_rgba(0,0,0,0.2)] 
                        hover:shadow-[inset_0_3px_6px_rgba(0,0,0,0.4),0_4px_8px_rgba(14,165,233,0.15),0_0_15px_rgba(0,0,0,0.3)] 
                        focus:shadow-[inset_0_3px_6px_rgba(0,0,0,0.4),0_0_20px_rgba(14,165,233,0.3),0_8px_16px_rgba(0,0,0,0.2)] ${errors.otp ? 'border-red-400/60 ring-2 ring-red-400/30' : 'border-slate-600/70 hover:border-sky-500/40'
                                                }`}
                                            style={{ textShadow: '0 0 10px rgba(255, 255, 255, 0.1)' }}
                                        />
                                        {errors.otp && (
                    ...
                    hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                        ...
                        focus:shadow-[inset_0_3px_6px_rgba(0,0,0,0.4),0_0_20px_rgba(34,197,94,0.3),0_8px_16px_rgba(0,0,0,0.2)] ${errors.newPassword ? 'border-red-400/60 ring-2 ring-red-400/30' : 'border-slate-600/70 hover:border-green-500/40'
                                                }`}
                                            style={{ textShadow: '0 0 10px rgba(255, 255, 255, 0.1)' }}
                                        />
                                        <button
                        ...
                        shadow-[inset_0_3px_6px_rgba(0,0,0,0.4),0_2px_4px_rgba(34,197,94,0.1),0_0_10px_rgba(0,0,0,0.2)] 
                        hover:shadow-[inset_0_3px_6px_rgba(0,0,0,0.4),0_4px_8px_rgba(34,197,94,0.15),0_0_15px_rgba(0,0,0,0.3)] 
                        focus:shadow-[inset_0_3px_6px_rgba(0,0,0,0.4),0_0_20px_rgba(34,197,94,0.3),0_8px_16px_rgba(0,0,0,0.2)] ${errors.confirmPassword ? 'border-red-400/60 ring-2 ring-red-400/30' : 'border-slate-600/70 hover:border-green-500/40'
                                                }`}
                                            style={{ textShadow: '0 0 10px rgba(255, 255, 255, 0.1)' }}
                                        />
                                        <button
